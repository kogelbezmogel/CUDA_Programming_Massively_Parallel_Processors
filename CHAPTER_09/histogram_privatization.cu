...
__global__ void histogram_privatization(int *A, int *H, int N, int BINS);
...
int main() {
    ...
    histogram_privatization<<<grid_size, block_size>>>(A_d, H_d, N, 10);
...
__global__ void histogram_privatization(int *A, int *H, int N, int BINS) {
    int bin_start = blockIdx.x * BINS;
    int id = blockIdx.x * blockDim.x + threadIdx.x;

    if(id < N) {
        int bin_id = A[id];
        atomicAdd(H + bin_start + bin_id, 1);
    }
    __syncthreads();


    if(blockIdx.x > 0) {
        // this loop makes shure that all bins will be covered even if (bins > block_size)
        for(int bin = threadIdx.x; bin <  BINS; bin += blockDim.x) {
            atomicAdd(H + bin, H[bin_start + bin]);
        }
    }
}
