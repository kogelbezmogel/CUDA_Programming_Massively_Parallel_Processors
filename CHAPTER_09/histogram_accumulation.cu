...
__global__ void histogram_accumulation(int *A, int *H, int N);
...
int main() {
    ...
    histogram_accumulation<<<grid_size, block_size>>>(A_d, H_d, N);
...
__global__ void histogram_accumulation(int *A, int *H, int N) {
    ...
    // variables for acccumulation mechanism
    int accu = 0;
    int last_bin = 0;
    int curr_bin = 0;
    ...
    for(int id = id_start; id < (blockIdx.x+1) * blockDim.x * CFACTOR; id += blockDim.x) {
        if(id < N) {
            ...
            if(last_bin == curr_bin) {
                accu += 1;
            } else {
                atomicAdd(hist_sh + last_bin, accu);
                accu = 1;
                last_bin = curr_bin;
            }
        }
    }
    // flushing last accumulation
    atomicAdd(hist_sh + last_bin, accu);
    __syncthreads();
