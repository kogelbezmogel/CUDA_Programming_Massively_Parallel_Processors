...
void __global__ stencil3D_coarsed(float *in, float *out,float *W, int N, int K, int M, int rank){
    ...
    if(pla_ite - 1 >= 0 && pla_ite - 1 < M) {
        if(row >= 0 && row < N && col >= 0 && col < K) {
            in_prev = in[(pla_ite-1) * N*K + row * K + col];
        }
    }
    ...
    for(pla_ite; pla_ite < M-1; ++pla_ite) {
        if(pla_ite + 1 >= 0 && pla_ite + 1 < M && row < N && col < K) {
            in_next = in[(pla_ite+1) * N*K + row * K + col];
        }
        __syncthreads();
        ...
        __syncthreads();
        in_prev = in_curr[row][col];
        in_curr[row][col] = in_next;
    }
...
// In book author uses additional in_curr register variable to reuse value of middle point fow W[0]